...
| ~~`startFrom`/`startTime`/`latestTime`/`firstTime` keywords + numeric time-dir naming~~ **fixed 2026-09-01, §10 Tier 3** | `provenance_inputs.py:90,119-155` | was: even having declared `openfoam.control_dict`, the *keyword vocabulary* inside it was still OpenFOAM's. Now lives only in `compatibility.legacy_selected_start_time`, the documented default behind `CaseIntrospectionCapability.selected_start_time()` — a plugin overrides the whole computation, not just the file lookup. **Superseded 2026-09-26 (spec A2c, final review M13):** `legacy_selected_start_time`/`CaseIntrospectionCapability.selected_start_time()` are gone; the OpenFOAM layer's own `openfoam.time_selection.selected_start_time` (behind `CaseProvenanceCapability.input_roots`/`get_input_roots`) is now the only place this logic lives, and it reads its instance-directory rule from the stack's merged `CaseRuntimeConventions.instance_directory_pattern` rather than a bare `float()` (final review M5) |
...
| ~~`--openfoam-bashrc` CLI flag + `openfoam_bashrc` parameter~~ **fixed 2026-09-02, §10 Tier 3** | `cli.py`, threaded through 11 files across all three packages once traced fully (larger than the original "6 files" estimate) | was: a FEniCS user types `--openfoam-bashrc`; the same capability was internally inconsistent, `load()` taking `explicit_bashrc` while `diagnostics()` took `openfoam_bashrc` for the identical concept. Renamed to `explicit_bashrc` everywhere in core and at the `EnvironmentPreflightCapability`/`SolverPluginOptionalHooks` boundary — not a new third name, `explicit_bashrc` was already `load()`'s and `omnidriver-openfoam`'s own established spelling, confirmed by checking `openfoam_environment.py`'s `_candidate_bashrcs`/`load_openfoam_environment`, which already used it. CLI flag is now `--environment-bashrc`. **2026-09-02, pre-publication cleanup:** the `--openfoam-bashrc` flag and `make_spec(openfoam_bashrc=...)`/its `cases[i]` JSON-key equivalent were first shipped as deprecated aliases, then removed outright the same day — no external callers exist yet to protect, and pre-publication is the cheapest moment to drop a name rather than carry it forward. `dict_builder.py`'s `build_and_launch(openfoam_bashrc=...)` keeps its own name (that's cardiacfoam's own correct vocabulary) but now translates it to `explicit_bashrc` at its one call into core's `make_spec()`. The plugin-hook-level keyword (`get_environment_diagnostics(..., explicit_bashrc=...)`) was renamed without a compatibility shim from the start -- only the test double implements that hook today, no real plugin. Files correctly kept saying "openfoam" throughout: `omnidriver-openfoam`'s own internal bashrc-discovery vocabulary (`discover_openfoam_bashrc`, `OPENFOAM_BASHRC` env var, `get_openfoam_bashrc` plugin-author hook) and `omnidriver-cardiacfoam`'s `build_and_launch(openfoam_bashrc=...)` — both packages are legitimately about OpenFOAM | **2026-09-26 (topic A, A1):** `explicit_bashrc` / `--environment-bashrc` became `environment_source` / `--environment-source`: one opaque value core passes to the plugin's environment hooks and never reads. The OpenFOAM layer sources it as a bashrc (its own helpers now take `bashrc_path`); openCARP ignores it. `generic_case` no longer stores it (it was dead data). No alias.
...
| ~~`cardiacfoam_monorepo_root()` — zero call sites~~ | **live.** Zero callers *in core*; real callers in both sibling packages' `conftest.py` and in root-level `scripts/`. Only the stale `utility_catalog.UTILITIES_ROOT` citation in its docstring was wrong, and that is fixed. **Moved 2026-09-26 (spec A6, final review M13):** this row's own location (`specs/paths.py`) is now stale too -- the function lives in `omnidriver.cardiacfoam.monorepo` |
...
**2026-09-26 (topic A, A2):** the Tier 3 "bare optional hooks" `selected_start_time` and `decomposition_dirname_prefix` are gone. Core now reads only plugin-declared vocabulary: `CaseRuntimeConventions.instance_directory_pattern`/`preserved_instance_names`/`replica_directory_globs`, the `{instance}` placeholder, and `CaseProvenanceCapability.input_roots`, through which the OpenFOAM layer declares its start time and replicas. The run-document schema's `DataArtifact.time_indexed` key is renamed `instance_indexed` (`schemas/run-document.json`, `core.runtime.models.DataArtifact`); `core.utility_catalog.UtilityManifest`'s own copy of the same field, read from `utility.manifest.toml`'s `produces` entries, is renamed the same way. Neither old key is translated: `load_run_document` and `data_artifact_from_json` both refuse it by name (R2 fix, finding M1).
