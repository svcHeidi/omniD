...
TREE_ENTRIES: Final[tuple[DictEntry, ...]] = (
    ...
    DictEntry(
        ...
        dynamic_path=True,
        notes="'endocardial': terminals stay on the grown tree. 'transmural': each terminal marches inward toward a target sampled from [extension.dMin, extension.dMax]; requires the extension sub-block.",
    ),
    DictEntry(
        ...
        notes="'straightSegment' samples one inward direction; 'gradientFollow' re-evaluates it every step and produces fewer marches that exit the mesh (README). Both require the endo=0/epi=1 transmural convention.",
    ),
    DictEntry(
        driver_path="$PURKINJE_TREE.<ventKey>.extension.dMin",
        description="Minimum normalized transmural depth (0=endo) sampled as a terminal-extension target.",
        source_refs=(_TREE_SOURCE, _TREE_README),
        value_kind="scalar",
        dynamic_path=True,
        applicable_when={"$PURKINJE_TREE.<ventKey>.terminalModel": ("transmural",)},
        required_when={"$PURKINJE_TREE.<ventKey>.terminalModel": ("transmural",)},
        constraints=("Must satisfy transmuralLowerValue <= dMin <= dMax <= transmuralUpperValue (generatePurkinjeTree.C's extension.dMin/dMax transmural-range guard).",),
    ),
    DictEntry(
        driver_path="$PURKINJE_TREE.<ventKey>.extension.dMax",
        description="Maximum normalized transmural depth sampled as a terminal-extension target; the effective depth ceiling since the march stops as soon as it crosses the sampled target.",
        source_refs=(_TREE_SOURCE, _TREE_README),
        value_kind="scalar",
        dynamic_path=True,
        applicable_when={"$PURKINJE_TREE.<ventKey>.terminalModel": ("transmural",)},
        required_when={"$PURKINJE_TREE.<ventKey>.terminalModel": ("transmural",)},
        constraints=("Must satisfy transmuralLowerValue <= dMin <= dMax <= transmuralUpperValue; see dMin sibling entry.",),
    ),
    DictEntry(
