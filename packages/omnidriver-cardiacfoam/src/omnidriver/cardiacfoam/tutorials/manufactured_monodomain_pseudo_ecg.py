...
def _plan_case(
    ...
    *,
    ...
    """`TutorialSpec.plan_case` (Phase 3 Task 6, completed 2026-09-23 per the
    decision "a parameter asserts a final state, not only a value") --
    **full migration** of the electroProperties portion Task 6 itself
    reported as resisting: the ECG electrode positions were written
    `add_if_missing=True` (an upsert -- now `resolve_electro_property_ensure`,
    `operation="ensure"`, catalog-addressed the same way a `set` is, via the
    declared `dynamic_path` entry
    `ecgDomains.<name>.electrodePositions.<electrode>`) and, when
    `ecg_enabled` is False, the whole `ecgDomains` dict is conditionally
    *removed* (`remove_electro_property_dict`) to clean up a block a
    previous, ECG-enabled case run against the same reused `case_root` may
    have left behind -- now a `plan_dict_block` raw target
    (`operation="remove"`), the same "not a `ParameterAssignment`, a whole
    sub-dictionary has no single `key_path`" shape Task 4's hex rewrite and
    `manufactured_bath_bidomain`'s own `ecgDomains` insert already have,
    folded into this function's existing `commit_case_overrides` call
    through `extra_targets` alongside the block-mesh target.
    ...
    **Corrected 2026-09-23 (Phase 3 Task 7):** the tet branch's overlay
    ...
    if ecg_enabled:
        ...
        electrode_scope = (electro_properties_scope, "ecgDomains", "ECG", "electrodePositions")
        for electrode_name, electrode_position in electrodes.items():
            electrode_parameters.append(resolve_electro_property_ensure(
                electro_properties, electrode_name, electrode_position,
                document=electro_document, scope=electrode_scope,
            ))
    ...
    record = commit_case_overrides(
        case_root,
        parameters=merge_assignments(control_dict_parameters, electro_parameters, physics_parameters),
