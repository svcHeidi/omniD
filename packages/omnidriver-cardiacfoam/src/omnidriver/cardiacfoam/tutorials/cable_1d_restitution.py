        ...
        metadata={
            ...
            "expected_artifacts": [
                ...
                DataArtifact(
                    ...
                    path_pattern=f"{defaults.OUTPUT_DIR_NAME}/*_restitution.csv",
                    format="csv",
                    description=(
                ...
                DataArtifact(
                    ...
                    path_pattern=f"{defaults.OUTPUT_DIR_NAME}/*_events.csv",
                    format="csv",
            ...
            "workflow_dag": {
                "steps": [
                    ...
                    },
                    {
                        ...
                        # A case-relative literal, deliberately NOT str(output_dir).
                        # sweep_runner rewrites output_dir_name to "." for a
                        # staged case, so a spec-time absolute path diverges
                        # from where this step actually runs. The step's cwd is
                        # the case root, and the postprocessor resolves this
                        # against it, so the location is the same in `run` and
                        # `sweep-run` -- which is what lets one path_pattern
                        # below match in both.
                        "args": ["--output-dir", defaults.OUTPUT_DIR_NAME],
                        "depends_on": ["run"],
                        # These must be claimed here even though Allrun.post is
                        # an auxiliary command. An expected artifact that no
                        # step claims is auto-credited by core's workflow
                        # builder to the last *solver* step -- see the
                        # unclaimed_artifacts branch in core/runtime/workflow.py
                        # -- and `run` would then be failed for not producing
                        # postprocessing output it never writes.
                        "produces": [
