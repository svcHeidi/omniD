...
def _template_path() -> Path:
    ...
    Tier 2: bundled package fixture — shipped inside
        ``omnidriver.cardiac``'s own ``fixtures/template/constant/electroProperties``
    ...
    for parent in current.parents:
        ...
        if candidate.exists():
            return candidate
    # Tier 2: bundled fixture co-located with the installed omnidriver.cardiac package
    import omnidriver.cardiac

    fixture = (
        Path(omnidriver.cardiac.__file__).parent
...
class TestMakeSpecDirectRun(unittest.TestCase):
    ...
    def test_spec_accepts_solver_command(self) -> None:
        import tempfile
        from omnidriver.cardiac.tutorials.generic_case import make_spec
