...
| **licensing** | **still fully open, re-measured 2026-09-02.** No `LICENSE` file anywhere in the repo, no `license` field in any of the three `pyproject.toml`. GPL/"This file is part of cardiacFoam" headers: **58 of 71 files in core** (`packages/omnidriver/src`) — worse than this section's original "55 of ~74" estimate, not better — plus 21/35 in `omnidriver-openfoam` and 81/143 in `omnidriver-cardiacfoam` (259 files repo-wide; the cardiacfoam-package count is expected, since that package's own content genuinely is cardiacFoam-derived — core's and openfoam's are not). `packages/omnidriver/README.md` still claims "zero OpenFOAM-specific vocabulary" and links to `../../ARCHITECTURE.md`, unresolved off-repo. Given the stated goal of a publication-ready release, this is the single largest concrete blocker left in this document — everything else in this section is either resolved or a narrow, bounded gap. |
